...
help:
	...
	@echo "  make check-public-release-paper-route-entry"
	@echo "                    Check public release paper route entry"
	@echo "  make check-public-release-paper-route"
	@echo "                    Check public release paper route"
	@echo "  make check-public-release-paper-route-entry-projections"
	@echo "                    Check public release paper route entry projections"
	...
	@echo "  make check-public-release-paper-ready-release"
	@echo "                    Check public release paper-ready release"
	@echo "  make check-public-release-export-table"
	@echo "                    Check public release export table"
	@echo "  make check-public-release-acceptance-certificate"
	@echo "                    Check public release acceptance certificate"
	@echo "  make check-public-release-one-click-entry"
	@echo "                    Check public release one-click entry"
	@echo "  make check-public-release-final-index"
	@echo "                    Check public release final index"
	@echo "  make check-public-release-homepage-index"
	@echo "                    Check public release homepage index"
	@echo "  make check-public-release-quick-start"
	@echo "                    Check public release quick start"
	@echo "  make check-public-release-sync-summary"
	@echo "                    Check public release sync summary"
...
check-public-release-paper-route-entry:
	@command -v rg >/dev/null || \
		(echo "Missing rg: install ripgrep before checking public release paper route entry." && exit 1)
	@if ! rg -q 'public release paper route entry check: `make check-public-release-paper-route-entry`' README.md; then \
		echo "README build status summary does not name the public release paper route entry check."; \
		exit 1; \
	fi
	@if ! sed -n '/^Current top-level entry points:/,/^## Contents/p' README.md | rg -q 'type_theory_atlas_public_release_paper_route_holds'; then \
		echo "README overview does not name the public release paper route entry."; \
		exit 1; \
	fi
	@if ! sed -n '/^## Build Status Summary/,/^The expected verification story is:/p' README.md | rg -q 'type_theory_atlas_public_release_paper_route_holds'; then \
		echo "README build status summary does not name the public release paper route entry."; \
		exit 1; \
	fi
	@missing=0; \
	for entry in \
		...
		type_theory_atlas_public_release_paper_route_holds \
		type_theory_atlas_public_release_paper_route_gives_certificate; do \
		if ! rg -q "^(Theorem|Corollary|Record|Definition|Lemma) $${entry}\\b" theories/Atlas/Metatheory.v; then \
			echo "Missing public release paper route entry in theories/Atlas/Metatheory.v: $$entry"; \
			missing=1; \
		fi; \
		if ! rg -q "$${entry}" README.md; then \
			echo "Missing public release paper route entry in README.md: $$entry"; \
			missing=1; \
		fi; \
	done; \
	if [ "$$missing" -ne 0 ]; then exit 1; fi; \
	echo "Public release paper route entry is documented and present in Metatheory.v."

check-public-release-paper-route:
	@command -v rg >/dev/null || \
		(echo "Missing rg: install ripgrep before checking public release paper route." && exit 1)
	@if ! rg -q 'public release paper route check: `make check-public-release-paper-route`' README.md; then \
		echo "README build status summary does not name the public release paper route check."; \
		exit 1; \
	fi
	@if ! sed -n '/^Current top-level entry points:/,/^## Contents/p' README.md | rg -q 'type_theory_atlas_public_release_paper_route_certificate_holds'; then \
		echo "README overview does not name the public release paper route certificate."; \
		exit 1; \
	fi
	@if ! sed -n '/^## Build Status Summary/,/^The expected verification story is:/p' README.md | rg -q 'type_theory_atlas_public_release_paper_route_certificate_holds'; then \
		echo "README build status summary does not name the public release paper route certificate."; \
		exit 1; \
	fi
	@missing=0; \
	for entry in \
		...
		type_theory_atlas_public_release_paper_route_gives_stage6_metatheory \
		type_theory_atlas_public_release_paper_route_gives_paper_statement; do \
		if ! rg -q "^(Theorem|Corollary|Record|Definition|Lemma) $${entry}\\b" theories/Atlas/Metatheory.v; then \
			echo "Missing public release paper route entry in theories/Atlas/Metatheory.v: $$entry"; \
			missing=1; \
		fi; \
	...
	printf '%s\n' \
		...
		type_theory_atlas_public_release_paper_route_gives_stage5_translation_reliability \
		type_theory_atlas_public_release_paper_route_gives_stage6_metatheory \
		type_theory_atlas_public_release_paper_route_gives_paper_statement > "$$expected"; \
	sed -n '/^Corollary type_theory_atlas_public_release_paper_route_gives_stage1_unified_syntax/,/^Corollary type_theory_atlas_public_release_paper_route_gives_paper_statement/p' theories/Atlas/Metatheory.v | \
		grep -Eo 'type_theory_atlas_public_release_paper_route_gives_(stage[0-9][A-Za-z0-9_]*|paper_statement)' > "$$coq_actual"; \
	sed -n '/^- Public release paper route entry and certificate:/,/^- Build status checks:/p' README.md | \
		grep -Eo '`type_theory_atlas_public_release_paper_route_gives_(stage[0-9][A-Za-z0-9_]*|paper_statement)`' | \
		tr -d '`' > "$$readme_actual"; \
	if ! diff -u "$$expected" "$$coq_actual"; then \
		echo "Coq public release paper route projections are not in the expected order."; \
		rm -f "$$expected" "$$coq_actual" "$$readme_actual"; \
		exit 1; \
	fi; \
	if ! diff -u "$$expected" "$$readme_actual"; then \
		echo "README public release paper route projections are not in the expected order."; \
		rm -f "$$expected" "$$coq_actual" "$$readme_actual"; \
		exit 1; \
	fi; \
	rm -f "$$expected" "$$coq_actual" "$$readme_actual"; \
	echo "Public release paper route names and projections match across Coq and README."

check-public-release-paper-route-entry-projections:
	@command -v rg >/dev/null || \
		(echo "Missing rg: install ripgrep before checking public release paper route entry projections." && exit 1)
	@if ! rg -q 'public release paper route entry projection check: `make check-public-release-paper-route-entry-projections`' README.md; then \
		echo "README build status summary does not name the public release paper route entry projection check."; \
		exit 1; \
	fi
	@expected=$$(mktemp); coq_actual=$$(mktemp); readme_actual=$$(mktemp); \
	printf '%s\n' \
	...
	if ! diff -u "$$expected" "$$readme_actual"; then \
		echo "README public release paper route entry projections are not in the expected order."; \
		rm -f "$$expected" "$$coq_actual" "$$readme_actual"; \
		exit 1; \
	fi; \
	rm -f "$$expected" "$$coq_actual" "$$readme_actual"; \
	echo "Public release paper route entry projections match across Coq and README."
...
check-public-homepage-summary:
	...
	for phrase in \
		...
		'type_theory_atlas_public_release_homepage_index_holds' \
		'type_theory_atlas_public_release_quick_start_holds' \
		'type_theory_atlas_public_release_sync_summary_holds'; do \
...
check-public-release-citation:
	...
	@if ! sed -n '/^## Public Release Citation/,/^## Build Status Summary/p' README.md | rg -q 'type_theory_atlas_public_release_paper_route_holds'; then \
	...
	@if ! sed -n '/^## Public Release Citation/,/^## Build Status Summary/p' README.md | rg -q 'type_theory_atlas_public_release_export_table_holds'; then \
		echo "Public release citation does not name the export table."; \
		exit 1; \
	fi
	@if ! sed -n '/^## Public Release Citation/,/^## Build Status Summary/p' README.md | rg -q 'type_theory_atlas_public_release_acceptance_certificate_holds'; then \
		echo "Public release citation does not name the release acceptance certificate."; \
		exit 1; \
	fi
	@if ! sed -n '/^## Public Release Citation/,/^## Build Status Summary/p' README.md | rg -q 'type_theory_atlas_public_release_one_click_entry_holds'; then \
		echo "Public release citation does not name the one-click release entry."; \
		exit 1; \
	fi
	@if ! sed -n '/^## Public Release Citation/,/^## Build Status Summary/p' README.md | rg -q 'type_theory_atlas_public_release_final_index_holds'; then \
		echo "Public release citation does not name the final release index."; \
		exit 1; \
	fi
	@if ! sed -n '/^## Public Release Citation/,/^## Build Status Summary/p' README.md | rg -q 'type_theory_atlas_public_release_homepage_index_holds'; then \
		echo "Public release citation does not name the homepage release index."; \
		exit 1; \
	fi
	@if ! sed -n '/^## Public Release Citation/,/^## Build Status Summary/p' README.md | rg -q 'type_theory_atlas_public_release_quick_start_holds'; then \
		echo "Public release citation does not name the quick-start release theorem."; \
		exit 1; \
	fi
	@if ! sed -n '/^## Public Release Citation/,/^## Build Status Summary/p' README.md | rg -q 'type_theory_atlas_public_release_sync_summary_holds'; then \
		echo "Public release citation does not name the sync summary."; \
		exit 1; \
	fi
	@echo "Public release citation snippet names the title, homepage, Coq release manifest, complete theorem, paper route theorem, citation certificate, export table, acceptance certificate, one-click entry, final index, homepage index, quick start, and sync summary."
...
check-public-release-citation-sync:
	...
	{ \
		...
		sed -n 's/^Theorem \(type_theory_atlas_public_release_paper_ready_release_holds\) :/\1/p' theories/Atlas/Metatheory.v; \
		sed -n 's/^Theorem \(type_theory_atlas_public_release_export_table_holds\) :/\1/p' theories/Atlas/Metatheory.v; \
		sed -n 's/^Theorem \(type_theory_atlas_public_release_acceptance_certificate_holds\) :/\1/p' theories/Atlas/Metatheory.v; \
		sed -n 's/^Theorem \(type_theory_atlas_public_release_one_click_entry_holds\) :/\1/p' theories/Atlas/Metatheory.v; \
		sed -n 's/^Theorem \(type_theory_atlas_public_release_final_index_holds\) :/\1/p' theories/Atlas/Metatheory.v; \
		sed -n 's/^Theorem \(type_theory_atlas_public_release_homepage_index_holds\) :/\1/p' theories/Atlas/Metatheory.v; \
		sed -n 's/^Theorem \(type_theory_atlas_public_release_quick_start_holds\) :/\1/p' theories/Atlas/Metatheory.v; \
		sed -n 's/^Theorem \(type_theory_atlas_public_release_sync_summary_holds\) :/\1/p' theories/Atlas/Metatheory.v; \
	} | sort -u > "$$expected"; \
	{ \
		sed -n '/^## Homepage Summary/,/^## Public Release Citation/p' README.md | \
			grep -Eo '(Final Coq entry point|Paper route entry point|Paper route certificate): `[^`]+`' | \
		...
		sed -n '/^## Public Release Citation/,/^## Build Status Summary/p' README.md | \
			grep -Eo 'Coq paper-ready release: [A-Za-z_][A-Za-z0-9_]*' | \
			sed 's/Coq paper-ready release: //'; \
		sed -n '/^## Public Release Citation/,/^## Build Status Summary/p' README.md | \
			grep -Eo 'Coq export table: [A-Za-z_][A-Za-z0-9_]*' | \
			sed 's/Coq export table: //'; \
		sed -n '/^## Public Release Citation/,/^## Build Status Summary/p' README.md | \
			grep -Eo 'Coq release acceptance: [A-Za-z_][A-Za-z0-9_]*' | \
			sed 's/Coq release acceptance: //'; \
		sed -n '/^## Public Release Citation/,/^## Build Status Summary/p' README.md | \
			grep -Eo 'Coq one-click release: [A-Za-z_][A-Za-z0-9_]*' | \
			sed 's/Coq one-click release: //'; \
		sed -n '/^## Public Release Citation/,/^## Build Status Summary/p' README.md | \
			grep -Eo 'Coq final index: [A-Za-z_][A-Za-z0-9_]*' | \
			sed 's/Coq final index: //'; \
		sed -n '/^## Public Release Citation/,/^## Build Status Summary/p' README.md | \
			grep -Eo 'Coq homepage index: [A-Za-z_][A-Za-z0-9_]*' | \
			sed 's/Coq homepage index: //'; \
		sed -n '/^## Public Release Citation/,/^## Build Status Summary/p' README.md | \
			grep -Eo 'Coq quick start: [A-Za-z_][A-Za-z0-9_]*' | \
			sed 's/Coq quick start: //'; \
		sed -n '/^## Public Release Citation/,/^## Build Status Summary/p' README.md | \
			grep -Eo 'Coq sync summary: [A-Za-z_][A-Za-z0-9_]*' | \
			sed 's/Coq sync summary: //'; \
	...
	if [ "$$(wc -l < "$$expected" | tr -d ' ')" -ne 14 ]; then \
		echo "Coq public release manifest/complete/paper-route/citation/paper-ready/export-table/acceptance/one-click/final-index/homepage-index/quick-start/sync-summary declarations are missing or ambiguous."; \
		rm -f "$$expected" "$$actual"; \
		exit 1; \
	fi; \
	if diff -u "$$expected" "$$actual"; then \
		echo "Public release citation references match the Coq release manifest, complete entries, paper route entries, citation certificate, paper-ready release, export table, acceptance certificate, one-click entry, final index, homepage index, quick start, and sync summary."; \
		rm -f "$$expected" "$$actual"; \
	else \
		echo "Public release citation references do not match the Coq release manifest, complete entries, paper route entries, citation certificate, paper-ready release, export table, acceptance certificate, one-click entry, final index, homepage index, quick start, and sync summary."; \
		rm -f "$$expected" "$$actual"; \
		exit 1; \
	fi
...
check-public-release-paper-ready-release:
	@command -v rg >/dev/null || \
		(echo "Missing rg: install ripgrep before checking the public release paper-ready release." && exit 1)
	@if ! rg -q 'public release paper-ready release check: `make check-public-release-paper-ready-release`' README.md; then \
		echo "README build status summary does not name the public release paper-ready release check."; \
		exit 1; \
	fi
	@missing=0; \
	for entry in \
		...
		if ! rg -q "$${entry}" README.md; then \
			echo "Missing public release paper-ready release entry in README.md: $$entry"; \
			missing=1; \
		fi; \
	done; \
	if [ "$$missing" -ne 0 ]; then exit 1; fi; \
	echo "Public release paper-ready release names are documented and present in Metatheory.v."

check-public-release-export-table:
	@command -v rg >/dev/null || \
		(echo "Missing rg: install ripgrep before checking the public release export table." && exit 1)
	@if ! rg -q 'public release export table check: `make check-public-release-export-table`' README.md; then \
		echo "README build status summary does not name the public release export table check."; \
		exit 1; \
	fi
	@missing=0; \
	for entry in \
		...
		type_theory_atlas_public_release_export_table_holds \
		type_theory_atlas_public_release_export_table_gives_paper_ready_release \
		type_theory_atlas_public_release_export_table_gives_citation_certificate \
		type_theory_atlas_public_release_export_table_gives_manifest \
		type_theory_atlas_public_release_export_table_gives_complete \
		type_theory_atlas_public_release_export_table_gives_paper_route \
		type_theory_atlas_public_release_export_table_gives_paper_route_certificate \
		type_theory_atlas_public_release_export_table_gives_final_public_theorem \
		type_theory_atlas_public_release_export_table_gives_final_certificate \
		type_theory_atlas_public_release_export_table_gives_release_summary \
		type_theory_atlas_public_release_export_table_gives_paper_statement; do \
		if ! rg -q "^(Theorem|Corollary|Record|Definition|Lemma) $${entry}\\b" theories/Atlas/Metatheory.v; then \
			echo "Missing public release export table entry in theories/Atlas/Metatheory.v: $$entry"; \
			missing=1; \
		fi; \
		if ! rg -q "$${entry}" README.md; then \
			echo "Missing public release export table entry in README.md: $$entry"; \
			missing=1; \
		fi; \
	done; \
	if [ "$$missing" -ne 0 ]; then exit 1; fi; \
	echo "Public release export table names are documented and present in Metatheory.v."

check-public-release-acceptance-certificate:
	@command -v rg >/dev/null || \
		(echo "Missing rg: install ripgrep before checking the public release acceptance certificate." && exit 1)
	@if ! rg -q 'public release acceptance certificate check: `make check-public-release-acceptance-certificate`' README.md; then \
		echo "README build status summary does not name the public release acceptance certificate check."; \
		exit 1; \
	fi
	@missing=0; \
	for entry in \
		...
		type_theory_atlas_public_release_acceptance_certificate_holds \
		type_theory_atlas_public_release_acceptance_gives_export_table \
		type_theory_atlas_public_release_acceptance_gives_paper_ready_release \
		type_theory_atlas_public_release_acceptance_gives_citation_certificate \
		type_theory_atlas_public_release_acceptance_gives_manifest \
		type_theory_atlas_public_release_acceptance_gives_complete \
		type_theory_atlas_public_release_acceptance_gives_paper_route \
		type_theory_atlas_public_release_acceptance_gives_final_public_theorem \
		type_theory_atlas_public_release_acceptance_gives_final_certificate \
		type_theory_atlas_public_release_acceptance_gives_release_summary \
		type_theory_atlas_public_release_acceptance_gives_paper_statement; do \
		if ! rg -q "^(Theorem|Corollary|Record|Definition|Lemma) $${entry}\\b" theories/Atlas/Metatheory.v; then \
			echo "Missing public release acceptance certificate entry in theories/Atlas/Metatheory.v: $$entry"; \
			missing=1; \
		fi; \
		if ! rg -q "$${entry}" README.md; then \
			echo "Missing public release acceptance certificate entry in README.md: $$entry"; \
			missing=1; \
		fi; \
	done; \
	if [ "$$missing" -ne 0 ]; then exit 1; fi; \
	echo "Public release acceptance certificate names are documented and present in Metatheory.v."

check-public-release-one-click-entry:
	@command -v rg >/dev/null || \
		(echo "Missing rg: install ripgrep before checking the public release one-click entry." && exit 1)
	@if ! rg -q 'public release one-click entry check: `make check-public-release-one-click-entry`' README.md; then \
		echo "README build status summary does not name the public release one-click entry check."; \
		exit 1; \
	fi
	@missing=0; \
	for entry in \
		...
		type_theory_atlas_public_release_one_click_gives_export_table \
		type_theory_atlas_public_release_one_click_gives_manifest \
		type_theory_atlas_public_release_one_click_gives_complete \
		type_theory_atlas_public_release_one_click_gives_paper_route \
		type_theory_atlas_public_release_one_click_gives_paper_statement; do \
		if ! rg -q "^(Theorem|Corollary|Record|Definition|Lemma) $${entry}\\b" theories/Atlas/Metatheory.v; then \
			echo "Missing public release one-click entry in theories/Atlas/Metatheory.v: $$entry"; \
			missing=1; \
		fi; \
		if ! rg -q "$${entry}" README.md; then \
			echo "Missing public release one-click entry in README.md: $$entry"; \
			missing=1; \
		fi; \
	done; \
	if [ "$$missing" -ne 0 ]; then exit 1; fi; \
	echo "Public release one-click entry names are documented and present in Metatheory.v."

check-public-release-final-index:
	@command -v rg >/dev/null || \
		(echo "Missing rg: install ripgrep before checking the public release final index." && exit 1)
	@if ! rg -q 'public release final index check: `make check-public-release-final-index`' README.md; then \
		echo "README build status summary does not name the public release final index check."; \
		exit 1; \
	fi
	@missing=0; \
	for entry in \
		...
		type_theory_atlas_public_release_final_index_gives_release_summary \
		type_theory_atlas_public_release_final_index_gives_paper_statement; do \
		if ! rg -q "^(Theorem|Corollary|Record|Definition|Lemma) $${entry}\\b" theories/Atlas/Metatheory.v; then \
			echo "Missing public release final index entry in theories/Atlas/Metatheory.v: $$entry"; \
			missing=1; \
		fi; \
		if ! rg -q "$${entry}" README.md; then \
			echo "Missing public release final index entry in README.md: $$entry"; \
			missing=1; \
		fi; \
	done; \
	if [ "$$missing" -ne 0 ]; then exit 1; fi; \
	echo "Public release final index names are documented and present in Metatheory.v."

check-public-release-homepage-index:
	@command -v rg >/dev/null || \
		(echo "Missing rg: install ripgrep before checking the public release homepage index." && exit 1)
	@if ! rg -q 'public release homepage index check: `make check-public-release-homepage-index`' README.md; then \
		echo "README build status summary does not name the public release homepage index check."; \
		exit 1; \
	fi
	@missing=0; \
	for entry in \
		...
		type_theory_atlas_public_release_homepage_index_holds \
		type_theory_atlas_public_release_homepage_index_gives_final_index \
		type_theory_atlas_public_release_homepage_index_gives_one_click_entry \
		type_theory_atlas_public_release_homepage_index_gives_acceptance_certificate \
		type_theory_atlas_public_release_homepage_index_gives_manifest \
		type_theory_atlas_public_release_homepage_index_gives_complete \
		type_theory_atlas_public_release_homepage_index_gives_paper_route \
		type_theory_atlas_public_release_homepage_index_gives_release_summary \
		type_theory_atlas_public_release_homepage_index_gives_paper_statement; do \
		if ! rg -q "^(Theorem|Corollary|Record|Definition|Lemma) $${entry}\\b" theories/Atlas/Metatheory.v; then \
			echo "Missing public release homepage index entry in theories/Atlas/Metatheory.v: $$entry"; \
			missing=1; \
		fi; \
		if ! rg -q "$${entry}" README.md; then \
			echo "Missing public release homepage index entry in README.md: $$entry"; \
			missing=1; \
		fi; \
	done; \
	if [ "$$missing" -ne 0 ]; then exit 1; fi; \
	echo "Public release homepage index names are documented and present in Metatheory.v."

check-public-release-quick-start:
	@command -v rg >/dev/null || \
		(echo "Missing rg: install ripgrep before checking the public release quick start." && exit 1)
	@if ! rg -q 'public release quick start check: `make check-public-release-quick-start`' README.md; then \
		echo "README build status summary does not name the public release quick start check."; \
		exit 1; \
	fi
	@missing=0; \
	for entry in \
		...
		type_theory_atlas_public_release_quick_start_gives_complete \
		type_theory_atlas_public_release_quick_start_gives_paper_statement; do \
		if ! rg -q "^(Theorem|Corollary|Record|Definition|Lemma) $${entry}\\b" theories/Atlas/Metatheory.v; then \
			echo "Missing public release quick start entry in theories/Atlas/Metatheory.v: $$entry"; \
			missing=1; \
		fi; \
		if ! rg -q "$${entry}" README.md; then \
			echo "Missing public release quick start entry in README.md: $$entry"; \
			missing=1; \
		fi; \
	done; \
	if [ "$$missing" -ne 0 ]; then exit 1; fi; \
	echo "Public release quick start names are documented and present in Metatheory.v."

check-public-release-sync-summary:
	@command -v rg >/dev/null || \
		(echo "Missing rg: install ripgrep before checking the public release sync summary." && exit 1)
	@if ! rg -q 'public release sync summary check: `make check-public-release-sync-summary`' README.md; then \
		echo "README build status summary does not name the public release sync summary check."; \
		exit 1; \
	fi
	@missing=0; \
	for entry in \
		...
		type_theory_atlas_public_release_sync_summary_holds \
		type_theory_atlas_public_release_sync_summary_gives_quick_start \
		type_theory_atlas_public_release_sync_summary_gives_homepage_index \
		type_theory_atlas_public_release_sync_summary_gives_final_index \
		type_theory_atlas_public_release_sync_summary_gives_manifest \
		type_theory_atlas_public_release_sync_summary_gives_daily_report \
		type_theory_atlas_public_release_sync_summary_gives_complete \
		type_theory_atlas_public_release_sync_summary_gives_release_summary \
		type_theory_atlas_public_release_sync_summary_gives_paper_statement; do \
		if ! rg -q "^(Theorem|Corollary|Record|Definition|Lemma) $${entry}\\b" theories/Atlas/Metatheory.v; then \
			echo "Missing public release sync summary entry in theories/Atlas/Metatheory.v: $$entry"; \
			missing=1; \
		fi; \
		if ! rg -q "$${entry}" README.md; then \
			echo "Missing public release sync summary entry in README.md: $$entry"; \
			missing=1; \
		fi; \
	done; \
	if [ "$$missing" -ne 0 ]; then exit 1; fi; \
	echo "Public release sync summary names are documented and present in Metatheory.v."
...
check-public-readme-release-package:
	...
	@$(MAKE) check-public-release-paper-route-entry
	@$(MAKE) check-public-release-paper-route
	@$(MAKE) check-public-release-paper-route-entry-projections
	...
	@$(MAKE) check-public-release-paper-ready-release
	@$(MAKE) check-public-release-export-table
	@$(MAKE) check-public-release-acceptance-certificate
	@$(MAKE) check-public-release-one-click-entry
	@$(MAKE) check-public-release-final-index
	@$(MAKE) check-public-release-homepage-index
	@$(MAKE) check-public-release-quick-start
	@$(MAKE) check-public-release-sync-summary
	...
	@if ! sed -n '/^## Homepage Summary/,/^## Build Status Summary/p' README.md | rg -q 'type_theory_atlas_public_release_paper_route_certificate_holds'; then \
		echo "Public README release package does not include the release paper route certificate."; \
		exit 1; \
	fi
	@if ! sed -n '/^## Homepage Summary/,/^## Build Status Summary/p' README.md | rg -q 'type_theory_atlas_public_release_paper_route_holds'; then \
		echo "Public README release package does not include the release paper route theorem."; \
		exit 1; \
	fi
	...
	@echo "Public README release package checks homepage summary, GitHub snippet, citation, citation sync, citation certificate, paper-ready release, export table, acceptance certificate, one-click entry, final index, homepage index, quick start, sync summary, manifest, release-complete certificate, release-complete entry projections, paper route entry, paper route, paper route entry projections, stage field/projection order, and verification entry."
...
check-public-release-final-package:
	...
	@echo "Final public release package checks release content, release-complete certificate, acceptance certificate, final index, homepage index, quick start, sync summary, source hygiene, release checklist, README release map, README navigation, expanded verification, help/README sync, and .PHONY/help sync."
