...
check-public-release-citation-sync:
	...
	if [ "$$(wc -l < "$$expected" | tr -d ' ')" -ne 7 ]; then \
		echo "Coq public release manifest/complete/paper-route/citation/paper-ready declarations are missing or ambiguous."; \
		rm -f "$$expected" "$$actual"; \
		exit 1; \
	fi; \
	if diff -u "$$expected" "$$actual"; then \
		echo "Public release citation references match the Coq release manifest, complete entries, paper route entries, citation certificate, and paper-ready release."; \
		rm -f "$$expected" "$$actual"; \
	else \
		echo "Public release citation references do not match the Coq release manifest, complete entries, paper route entries, citation certificate, and paper-ready release."; \
		rm -f "$$expected" "$$actual"; \
		exit 1; \
	fi
...
check-public-readme-release-package:
	...
	@echo "Public README release package checks homepage summary, GitHub snippet, citation, citation sync, citation certificate, paper-ready release, manifest, release-complete certificate, release-complete entry projections, paper route entry, paper route, paper route entry projections, stage field/projection order, and verification entry."
